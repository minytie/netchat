...
public class TcpSocketController {
    ...
    public static TcpSocket getAvailableSocketByPort(int port) {
        try {
            for (int i = 0; i < availableTcpSockets.size(); i++) {
                if (port == availableTcpSockets.get(i).getLocalPort()) {
                    return availableTcpSockets.get(i);
                }
            }
            return null;
        }
...
//        ParseFile.writeConfigNeighborsFile();
    ...
    public static boolean isAvailableSocket(int port) {
        for (int i = 0; i < availableTcpSockets.size(); i++) {
            if (port == availableTcpSockets.get(i).getLocalPort()) {
                return true;
            }
    ...
    public static void printLocalSocket() {
        System.out.println("Available Sockets:");
        for (TcpSocket socket : availableTcpSockets) {
            System.out.println("socket"+socket.getId()+":port"+socket.getLocalPort());
        }
